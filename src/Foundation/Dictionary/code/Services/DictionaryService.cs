...
namespace Thread.Foundation.Dictionary.Services
{
    public class DictionaryService
    {
        ...
        public string RenderTextDisplay(string text, string dictionaryDomain, string key)
        {
            ...
            var dictionaryItems = GetDictionaryEntries(dictionaryDomain);

            if (dictionaryItems == null || dictionaryItems.All(x=>x.Key != key))
                return text;

            Item currentItem = Context.Database.GetItem(dictionaryItems[key]);
        ...
        protected List<Item> GetSearchItems(Expression<Func<SearchResultItem, bool>> predicate)
        {
            ...
            {
              var enumerable = context.GetQueryable<SearchResultItem>()
                .Where(predicate)
                .Select(a => a.GetItem())
                .AsEnumerable();

              // Check for null items in case the index is out of date.
