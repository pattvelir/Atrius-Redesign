...
namespace AtriusHealth.Foundation.ResponsiveImages.Extensions
{
    public static class MediaItemExtensions
    {
        ...
        public static string GetSrcSetWidths(this MediaItem mediaItem, params int[] widths)
        {
            ...
            return GetSrcSetWidths(mediaItem.GetSrc(), widths);
        }
        ...
        /// <returns></returns>
        public static string GetSrcSetWidths(this string imageSrc, params int[] widths)
        {
            if (string.IsNullOrEmpty(imageSrc)) return string.Empty;

            var outputElements = new List<string>();
            foreach (int width in widths.OrderByDescending(w => w))
            {
                outputElements.Add($"{imageSrc.FormatImagePath(width)} {width}w");
            }
