...
Telerik.Web.UI.Editor.CommandList["InsertSitecorePlaceholder"] = function (commandName, editor, args) {
  addPlaceholderToEditor(editor, "l-rte-full");
};

Telerik.Web.UI.Editor.CommandList["InsertSitecorePlaceholderLeft"] = function (commandName, editor, args) {
  addPlaceholderToEditor(editor, "l-pull-left");
};

Telerik.Web.UI.Editor.CommandList["InsertSitecorePlaceholderRight"] = function (commandName, editor, args) {
  addPlaceholderToEditor(editor, "l-pull-right");
};

function addPlaceholderToEditor(editor, arg) {
  var selection = editor.getSelection();
  var parentElement = selection.getParentElement();

  var ancestorParagraph = parentElement.closest("p, h1, h2, h3 , h4, h5, h6");

  if (ancestorParagraph !== null) {
    parentElement = ancestorParagraph;
  }

  var placeholder = document.createElement("div");

  var cssClass = document.createAttribute("class");
  cssClass.value = "rich-text__embed";

  if (arg) {
    cssClass.value += " " + arg;
  }

  placeholder.setAttributeNode(cssClass);
  placeholder.appendChild(document.createTextNode("###PLACEHOLDER###"));

  if (editor.getSelectedElement().tagName === "BODY") {
    selection.pasteHtml(placeholder.outerHTML);
  } else {
    parentElement.parentNode.insertBefore(placeholder, parentElement);
  }
}
