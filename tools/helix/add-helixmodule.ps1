...
function Rename-Module
{
	...
	Rename-Folders -StartPath "$StartPath" -OldValue $config.TemplateNamespacePrefix -NewValue $config.NamespacePrefix
...
function Add-Projects
{
    ...
    Get-ChildItem -File -Path $ModulePath -Include "*$csprojExtension", "*$scprojExtension" -Recurse | ForEach-Object { $moduleNameInterface.AddFromFile("$($_.FullName)")}
...
function Add-Module
{
    ...
    try
    {
        ...
        $configJsonFile = Get-ChildItem -Path "$solutionRootFolder\tools\helix" -File -Filter "$addHelixModuleConfigFile" -Recurse | Select-Object -First 1 | Select-Object -ExpandProperty FullName
