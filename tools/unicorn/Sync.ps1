...
$unicornSecret = $secret
if(!$unicornSecret) {
    [xml]$xmlUnicorn = Get-Content -Path "$ScriptPath\..\..\src\Foundation\Serialization\code\App_config\Include\Unicorn.SharedSecret.config"
    $unicornSecret = $xmlUnicorn.configuration.sitecore.unicorn.authenticationProvider.SharedSecret
}

# SYNC ALL CONFIGURATIONS
Sync-Unicorn -ControlPanelUrl $url -SharedSecret $unicornSecret
