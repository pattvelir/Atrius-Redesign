...
module.exports = function createAtriusTasks(props) {
  const { projectRoot } = props;

  const buildDir = `${projectRoot}/src/Project/Atrius/code/assets/Atrius-build`;
  const frontendDir = `${projectRoot}/frontend/Atrius`;
  ...
  const browserSync = () =>
    runBrowserSyncTask({
      ...
      // server: {
      //   baseDir: `${projectRoot}/storybook-static-Atrius`,
      // },
      serveStatic: [
        {
          route: "/assets/Atrius-build",
          dir: `${buildDir}/`,
        },
      ],
      files: [`${buildDir}/**`, "!**/*.map"],
  ...
  const storybook = () =>
    runCommand(
      `${projectRoot}/node_modules/.bin/start-storybook --port 3030 --config-dir .storybook-Atrius --static-dir ./src/Project/Atrius/code`,
    );
