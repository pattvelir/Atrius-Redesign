...
module.exports = function createAtriusHealthTasks(props) {
  const { projectRoot } = props;

  const buildDir = `${projectRoot}/src/Project/AtriusHealth/code/assets/AtriusHealth-build`;
  const frontendDir = `${projectRoot}/frontend/AtriusHealth`;
  ...
  const browserSync = () =>
    runBrowserSyncTask({
      ...
      // server: {
      //   baseDir: `${projectRoot}/storybook-static-AtriusHealth`,
      // },
      serveStatic: [
        {
          route: "/assets/AtriusHealth-build",
          dir: `${buildDir}/`,
        },
      ],
      files: [`${buildDir}/**`, "!**/*.map"],
  ...
  const storybook = () =>
    runCommand(
      `${projectRoot}/node_modules/.bin/start-storybook --port 3030 --config-dir .storybook-AtriusHealth --static-dir ./src/Project/AtriusHealth/code`,
    );
